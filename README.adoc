...
While Fedora Silverblue is my daily driver, this repository is also compatible with Fedora Workstation.

To install `chezmoi` and apply this repository's dotfiles, use one of the following:

.rpm-ostree:
`rpm-ostree install --apply-live chezmoi && chezmoi init --apply 16levels`

.dnf:
`dnf -y install chezmoi && chezmoi init --apply 16levels`
